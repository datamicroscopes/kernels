...
travis_before_install:
	...
	(cd .travis && python before_install_microscopes_mixturemodel.py)
	(cd .travis && python before_install_microscopes_irm.py)
...
travis_install: 
	...
	(cd .travis && python install_microscopes_mixturemodel.py)
	(cd .travis && python install_microscopes_irm.py)
	mkdir -p build
